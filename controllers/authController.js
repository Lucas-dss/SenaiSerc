...
exports.register = async (req, res) => {
  ...
  try {
    ...
    Usuario.create({ email, senha: senhaCriptografada }, (err, result) => {
      if (err) {
      ...
      if (result.affectedRows === 1) {
        return res.redirect('/login'); // Após registro, redireciona para tela de login
      } else {
...
exports.login = (req, res) => {
  ...
  Usuario.findByEmail(email, async (err, results) => {
    ...
    // Redireciona para página de página principal
